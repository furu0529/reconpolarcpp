    ...
    if (EV%no_phot_multpoles) then
        ...
        clxg=ay(EV%g_ix)
        qg=ay(EV%g_ix+1)
        if (.not. EV%TightCoupling) pig=ay(EV%g_ix+2) !ここのpigが出力される
    end if
    ...
    if (associated(EV%OutputTransfer)) then
        ...
        EV%OutputTransfer(Transfer_tot_de) =  clxq !dgrho/grho_matter !changed by K.I.
        ...
        EV%OutputTransfer(Transfer_pig) = pig !wanna output Pi_gamma,add by 2020/6/26
    end if
